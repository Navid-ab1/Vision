...
def evaluate_model(data_dir, label_dir, num_folds=5, model_path='model.pth'):
    ...
    for fold in range(num_folds):
        model = CNNModel()
        model.to(device)
        fold_model_path = f'{model_path}_fold{fold+1}.pth'
        ...
        print(f"Fold {fold+1} - Accuracy (distance < 10 pixels): {accuracy * 100:.2f}%")

        # Use the correct batch of images for visualization
        sample_images, _ = next(iter(test_loader))
        visualize_results(sample_images.cpu().numpy(), all_true_points, all_predicted_points, save_path=f'visualizations_fold{fold+1}')
